...
def Utilization(df, Actual_Times = False, SD = False, ND = False, Travel = False, Absence_Type = False):
    ...
    for date in df['Date'].unique():
        ...
        travel_starts = day_window[['Travel To Start','Product','ID','Date','Lead Time','Fiscal Week','Fiscal Year','Type','Reason']]
        travel_finishes = day_window[['Travel To Finish','Product','ID','Date','Lead Time','Fiscal Week','Fiscal Year','Type','Reason']]
        ...
        travel_starts.columns = ['Event','Product','ID','Date','Lead Time','Fiscal Week','Fiscal Year','Type','Reason']
        travel_finishes.columns = ['Event','Product','ID','Date','Lead Time','Fiscal Week','Fiscal Year','Type','Reason']
        
        travel_starts['Actual Event'] = dc(travel_starts['Event'])
        travel_finishes['Actual Event'] = dc(travel_finishes['Event'])
        
        starts['Event Type'] = 'Start'
        finishes['Event Type'] = 'Finish'
        travel_starts['Event Type'] = 'Travel Start'
        travel_finishes['Event Type'] = 'Travel Finish'
        
        
        if not Travel:
            day_window = pd.concat([starts, finishes], sort = False, ignore_index = True)
        else:
            day_window = pd.concat([starts, finishes, travel_starts, travel_finishes], sort = False, ignore_index = True)
        
        
        day_window.sort_values('Event', ascending = True, inplace = True)
        day_window.reset_index(drop = True, inplace = True)
        del starts, finishes
        ...
        if day_window.index.tolist():
                    ...
                    if last_type == 'Start of Day':
                        ...
                        ### Finish --> Start
                            ...
                            if open_ids:
                                ...
                                last_open_event = open_event[-1]
                                
                                if last_open_type == 'Absence':
                                    absence_time.append(duration)
                                
                                elif last_open_type == 'Appointment':
                                    if last_open_lead_time == 0:
                                        sd_time.append(duration)
                                    elif last_open_lead_time == 1:
                                        nd_time.append(duration)
                                    else:
                                        other_appt_time.append(duration)
                        ...
                        ### Travel Start --> Start
                        elif last_event_type == 'Travel Start':
                            if last_type == 'Absence':
                                abs_travel_time.append(duration)
                            elif last_type == 'Appointment':
                                appt_travel_time.append(duration)
                        
                        
                        ### Travel Finish --> Start
                        elif last_event_type == 'Travel Finish':
                            if open_ids:
                                last_open_type = open_types[-1]
                                last_open_lead_time = open_lead_times[-1]
                                last_open_event = open_event[-1]
                                
                                if last_open_type == 'Absence' and (last_open_event == 'Start' or last_open_event == 'Finish'):
                                    absence_time.append(duration)
                                    
                                elif last_open_type == 'Absence' and (last_open_event == 'Travel Start' or last_open_event == 'Travel Finish'):
                                    abs_travel_time.append(duration)
                                    
                                elif last_open_type == 'Appointment' and (last_open_event == 'Start' or last_open_event == 'Finish'):
                                    if last_open_lead_time == 0:
                                        sd_time.append(duration)
                                    elif last_open_lead_time == 1:
                                        nd_time.append(duration)
                                ...
                                elif last_open_type == 'Appointment' and (last_open_event == 'Travel Start' or last_open_event == 'Travel Finish'):
                                    appt_travel_time.append(duration)
                        ...
                        if last_event_type == 'Start':
                            ...
                            if priority_type == 'Absence':
                                ...
                                if priority_lead_time == 0:
                                    sd_time.append(duration)
                                elif priority_lead_time == 1:
                                    nd_time.append(duration)
                                elif priority_lead_time > 1:
                                    other_appt_time.append(duration)
                            ...
                            if record_type == 'Absence':
                                ...
                                if record_lead_time == 0:
                                    sd_time.append(duration)
                                elif record_lead_time == 1:
                                    nd_time.append(duration)
                                elif priority_lead_time > 1:
                                    other_appt_time.append(duration)
            ...
            appt_travel_time = [x/(60*60) for x in appt_travel_time]
            abs_travel_time = [x/(60*60) for x in abs_travel_time]
            ...
            data['Appointment Travel Time'].append(sum(appt_travel_time))
            data['Absence Travel Time'].append(sum(abs_travel_time))
...
# =============================================================================
# query_list = [f"""
# SELECT CKSW_BASE__Resource__r.Name, CKSW_BASE__Resource__r.User_LDAP__c, CKSW_BASE__Appointment_Start__c, CreatedDate, CKSW_Actual_Start__c, CKSW_Actual_Finish__c, CKSW_BASE__Travel_Time_To__c, CKSW_BASE__Travel_Time_From__c, CKSW_BASE__Location__c, Opportunity__r.Product_Name__c, Name
...
# 
# f"""
# select Sales_Manager__c, Sales_Manager__r.Branch__r.Name, Sales_Manager__r.Owner.name
# from Store_Manager__c
# where Sales_Manager__r.LOB__c like '%{LOB}%'
# """,
# f"""
# select CKSW_BASE__Duration__c, Product__r.Name
# from CKSW_BASE__Service_Type__c
# where RecordType.Name like '%{LOB}%' and Product__r.Name != null
# """]
# 
# object_list = ['CKSW_BASE__Service__c','CKSW_BASE__Employee_Absence__c','Store_Manager__c','CKSW_BASE__Service_Type__c']
# 
# 
# 
...
# 
# 
# appointments.columns = ['SC','LDAP','Start','Created', 'Actual Start', 'Actual Finish','Travel To','Travel From','Location','Product','ID']
# absences.columns = ['SC','LDAP','Location','Start','Reason','Finish','Travel To','Travel From','ID']
# sms.columns = ['Location','Branch','SM']
# durations.columns = ['Duration','Product']
# appointments = appointments.merge(durations, on = 'Product', how = 'left')
# del durations
# 
# 
...
# appointments.drop(['Duration','Created'], axis = 1, inplace = True)
# 
# 
...
series = []
keys = ['Date','Total Day','Raw Free Time','Free Time Slots','SD Appt Time','ND Appt Time','Other Appt Time','Absence Time','Appointment Travel Time','Absence Travel Time']
for key in keys:
...
print(df.head())
# =============================================================================
# df.to_excel('DELETE ME TOO.xlsx', index = False)
# =============================================================================
