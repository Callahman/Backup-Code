...
naming_convention = ' Salesforce HC'
current_tech_headcount = None
...
queries = [f"""
select CKSW_HDMS_Market__r.Name, CKSW_HDMS_Market__r.CKSW_BASE__Parent_Location__r.name
from CKSW_BASE__Zip_Code__c
where CKSW_HDMS_Market__r.CKSW_BASE__Parent_Location__r.name != null and (not CKSW_HDMS_Market__r.CKSW_BASE__Parent_Location__r.name like '%cleveland%') and (not CKSW_HDMS_Market__r.CKSW_BASE__Parent_Location__r.name like '%dallas%')
"""]
objects = ['CKSW_BASE__Zip_Code__c']

           
df = SF_Dataframe(queries, objects)[0]
df.drop_duplicates(inplace = True)
df.columns = ['Market','Parent']

ignore_markets = []
df = df[~df['Parent'].isin(ignore_markets)]
...
for parent_location in parents:
    ...
    if target_productivity != None:
        market_location_name = parent_location.replace('/','-') + f' prod-{target_productivity}'
